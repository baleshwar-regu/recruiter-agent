...
def save_transcript(persona: str, transcript: list[tuple[str, str]]):
    with open(
        f"{TRANSCRIPTS_DIR}/{persona}_transcript.txt", "w", encoding="utf-8"
    ) as f:
...
def run_common_assertions(persona: str, transcript: list[tuple[str, str]]):
    ...
    assert (
        "current project" in flat_text or "describe your project" in flat_text
    ), f"[{persona}] Agent skipped project discussion"
    assert "sql" in flat_text, f"[{persona}] Agent did not ask SQL question"
    assert (
        "do you have any questions" in flat_text
    ), f"[{persona}] Agent skipped wrap-up question"
