...
def handle_invitee_created(payload: dict):
    ...
    logger.info(f"upsert_candidate_from_calendly INTERVIEW_SCHEDULED {name} {email} {phone} {scheduled_time}")
...
def handle_invitee_canceled(payload: dict):
    ...
    logger.info(f"upsert_candidate_from_calendly INTERVIEW_CANCELED {name} {email} {phone} scheduled_time None")
    candidate_id = upsert_candidate_from_calendly(
        name=name,
        email=email,
        phone=phone,
...
def extract_event_id(url: str) -> str:
    ...
    """
    path = urlparse(url).path  # => '/scheduled_events/3d5b9b31-f5bd-40a0-95f8-79a423dba88e'
